...
解释或修改字段前，运行 `osu-skin db-query "<命令名>" --type skin_ini --json`，核对 `section`、`value_type`、默认值、有效值和客户端。处理 `NoteImage#L` 或 `NoteBodyStyle` 时再读取 `mania-hold-body.md`。
...
| 命令 | 类型 | 语义/默认 | 诊断用途 |
|---|---|---|---|
| `ComboPrefix` | path | 连击数字前缀，默认 `score` | 检查 combo-0..9 图片 |
| `ComboOverlap` | integer | 连击数字重叠像素，默认 0 | 影响 combo 数字宽度 |
...
| `ColumnSpacing` | comma numbers / 0 | 列间透明间隔 |
| `ColumnLineWidth` | comma numbers / 2 | 分隔线宽度 |
| `StageSeparation` | number / 40 | 分割舞台间距；stable |
| `ComboPosition` | integer / 舞台中心 | combo 计数器高度 |
| `HitPosition` | integer / 402 | 判定线高度，影响 StageHint/LightingN/LightingL |
| `LightPosition` | integer / 413 | StageLight 高度 |
| `WidthForNoteHeightScale` | number / 最小列宽比例 | 列宽不同时统一 note 高度 |

列居中通常先计算总宽：

```text
总宽 = sum(ColumnWidth[i]) + sum(ColumnSpacing[i]) + 分隔线/舞台边界占用
```

不要只改 `ColumnStart` 而忽略逐列宽度、间距、StageLeft/Right 和目标分辨率。
...
| 命令 | 语义 |
|---|---|
| `NoteBodyStyle` | 通用长按体样式；lazer 有效值 0/2/3/4，值 1 不可解析 |
...
| `SplitStages` | 多键时是否分割舞台；键数大于 1 时可能由客户端强制 |
...
| `ScorePosition` | 判定图高度；stable 默认 325，lazer 默认 300 |
